...
import com.jfeat.org.services.domain.model.ExtOrgRelationDTO;
...
public class ExtOrgEndpoint {
    ...
    public ApiResult<Long> delete(
        ...
        return ApiResult.success(sysOrgService.extDelete(id));
    }
    ...
    public ApiResult<Long> update(
            ...
            @RequestBody @Validated ExtOrgDTO extOrgDTO) {
        return ApiResult.success(sysOrgService.extUpdate(extOrgDTO));
    }
    ...
    public ApiResult<List<ExtOrgRelationDTO>> list() {
        return ApiResult.success(sysOrgService.extList());
    }
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "同步成功", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "500", description = "同步失败，请检查外部数据源连接")
    })
    public ApiResult<Long> sync() {
        return ApiResult.success(sysOrgService.sync());
    }
}
